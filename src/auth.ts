...
// Sembunyikan halaman admin secara instan untuk mencegah konten terlihat sebelum dicek
...
declare global {
  interface Window {
    ...
    updateActivityTimeout?: ReturnType<typeof setTimeout> | null;
  }
}
...
// --- Fitur Inactivity Auto-Logout ---
const INACTIVITY_TIMEOUT = 5 * 60 * 1000; // Sesi hangus setelah 5 menit (dalam ms)

function updateLastActivity() {
    if (auth.currentUser) {
        localStorage.setItem('kataKita_lastActivity', Date.now().toString());
    }
}

function checkInactivity() {
    const lastActivity = localStorage.getItem('kataKita_lastActivity');
    if (lastActivity && auth.currentUser) {
        const timePassed = Date.now() - parseInt(lastActivity, 10);
        if (timePassed > INACTIVITY_TIMEOUT) {
            console.log("Sesi login daluwarsa karena tidak aktif.");
            signOut(auth).then(() => {
                localStorage.removeItem('kataKita_lastActivity');
                alert('Sesi Anda telah berakhir karena lama tidak membuka web/tidak ada aktivitas. Silakan login kembali.');
                if (window.location.pathname.includes('/admin')) {
            ...
            return true; // Menandakan sesi telah usai
        }
    }
    return false;
}

// Cek setiap 1 menit secara background
setInterval(checkInactivity, 60000);

// Update timestamp saat pengguna berinteraksi (dibatasi agar efisien kinerjanya)
['click', 'scroll', 'keypress', 'mousemove'].forEach(evt => {
    window.addEventListener(evt, () => {
        if (!window.updateActivityTimeout) {
            window.updateActivityTimeout = setTimeout(() => {
                updateLastActivity();
                window.updateActivityTimeout = null;
            }, 2000);
        }
    });
});
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        // Cek dulu apakah sesi dari buka tutup web sebelumnya sudah kadaluwarsa
        ...
        // Kembalikan visibilitas halaman admin jika diizinkan
        if (window.location.pathname.includes('/admin')) {
        ...
        // Redirect if on admin page without auth
        if (window.location.pathname.includes('/admin')) {
            window.location.replace('/');
        }
    }
